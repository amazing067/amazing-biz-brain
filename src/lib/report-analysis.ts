...
/**
 * 점수대별 단계 (저→고): 좋음(80+) → 보통(60~79) → 양호(40~59) → 주의(20~39) → 위험(0~19)
 * 동일 기준: report-images.ts getStageLabel, report-pages-34 recommendMsg, analyze-report-ai stageLabel
 */
...
export function analyzeReport(data: Record<string, unknown>): ReportAnalysis {
  ...
  // 동일 나이대 인지 위치 (로컬용 — 점수대별로 구체적 표현)
  const ageContext =
    total >= 90
      ? `같은 나이대에서 인지 기능이 상위 10% 내외로 매우 양호한 편으로 추정됩니다. 현재 수준 유지를 위해 규칙적인 생활과 사회 참여를 이어가시면 좋습니다.`
      : total >= 80
        ? `같은 나이대에서 인지 기능이 상위 20~30% 수준으로 추정됩니다. 정기 검진으로 변화를 살펴보시면 좋습니다.`
        : total >= 70
          ? `같은 나이대에서 인지지원등급 구간에 해당할 수 있으며, 경미한 인지 저하가 있을 수 있는 수준으로 보입니다. 6개월~1년 주기 검진으로 추이를 확인하시고, 필요 시 간병·장기요양 보험 상담을 권장합니다.`
          : total >= 60
            ? `같은 나이대에서 정상 범위에 가깝습니다. 일부 영역만 보완하면 좋은 수준이니, 주기적인 검진과 일상에서의 인지 활동(독서, 대화, 간단한 계산 등)을 권장합니다.`
            : total >= 50
              ? `같은 나이대 대비 일부 영역이 다소 낮게 나왔을 수 있습니다. 정밀 검사나 전문의 상담으로 원인을 확인하고, 점검해볼 영역에 맞춘 훈련·관리를 시작하시는 것을 권장합니다.`
              : total >= 40
                ? `같은 나이대 대비 여러 영역을 점검해 보시는 것이 좋습니다. 경도인지장애 가능성을 배제하기 위해 의료기관 인지 검사와 보험·돌봄 계획 상담을 함께 진행하시면 도움이 됩니다.`
                : total >= 20
                  ? `같은 나이대에서 정밀 검사·상담을 권장하는 구간으로 보입니다. 가능한 빨리 치매 클리닉 등 전문 기관을 방문하시고, 가족과 함께 돌봄·경제 부담에 대한 준비를 논의하시는 것을 권장합니다.`
                  : `같은 나이대에서 즉각적인 정밀 검사와 상담이 권장되는 구간입니다. 의료기관 방문과 함께 보험·지원 제도 상담을 받으시고, 가족과 돌봄 계획을 함께 정리하시기 바랍니다.`;

  // 치매·인지 위험 해석 (로컬용). 위험도는 총점만 사용(등급은 공단 판정이 아니므로 미반영).
  const grade = (data.grade as string) ?? '';
  const isEstimatedHighCare = /[1-5]등급|치매환자|경증|중등도/.test(grade);
  const dementiaRiskNote =
    total >= 80
      ? `현재 검진 결과만으로는 치매 고위험군으로 보기 어렵습니다. 1년에 1회 정도 정기 검진으로 변화를 관찰하시고, 수면·운동·사회 활동을 꾸준히 유지하시면 도움이 됩니다.`
      : total >= 70
        ? `선별 관점에서 저위험 구간에 해당합니다. 당장의 위험보다는 추이 관찰이 중요하므로, 6개월~1년 주기 검진과 간병·장기요양 보험 점검을 권장합니다.`
        : total >= 60
          ? `선별 관점에서 저위험에 가깝지만, 일부 영역을 보완할 여지가 있습니다. 주기적인 검진으로 변화를 확인하시고, 필요 시 간병·장기요양 보험 상담을 받아 보시는 것을 권장합니다.`
          : total >= 40
            ? `경도인지장애 가능성을 염두에 두고, 의료기관 인지 정밀 검사나 전문의 상담을 한 번 받아 보시는 것이 좋습니다. 검진 결과에 따라 보험·지원 제도도 함께 설계하시면 나중에 부담을 줄일 수 있습니다.`
            : isEstimatedHighCare
              ? `검진 결과상 해당 등급 구간으로 추정될 수 있으며, 실제 장기요양 등급은 공단 인정 심사 후 결정됩니다. 정밀 검사와 보험·지원 제도 상담을 함께 진행하시는 것을 권장합니다.`
              : `정밀 검사와 보험·지원 제도 상담을 함께 진행하시는 것을 권장합니다. 가족과 돌봄·경제 부담에 대해 미리 이야기해 두시면 좋습니다.`;

  // 이번 달 한 가지 행동 (로컬용 — 점수대별로 구체적 행동 제안)
  const oneActionThisMonth =
    total >= 80
      ? '이번 달에 실손·간병 보험 점검 또는 견적 1건 받아보기.'
      : total >= 70
        ? '이번 달에 간병·장기요양 보험 상담 1회 예약해 보시고, 다음 검진 일정(6개월~1년 후)을 미리 정해 두세요.'
        : total >= 60
